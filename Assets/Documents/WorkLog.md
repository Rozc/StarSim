...
### 再把伤害和韧性做一下就可以了，然后UI随便写写

明日计划：GM 分配 ID，进场，伤害与韧性，文档

            // 给自己上一个持续两回合的 Buff
            // 给自己以外的队友上一个不随回合减少的 Buff
            // 当自己的 Buff 结束时，移除队友身上的这个 Buff
            // TODO 可不可以在接收 Buff 的时候做处理
            // 对我方全体发送不随回合减少的结界 Buff
            // 对于罗刹自己，如果检测到这个 Buff 是结界 Buff，就把他替换成另一个
            // TODO 或者新增一种行动形式：Aoe 主从，可以认为是扩散的变种，然后把主 Buff 和从 Buff 分开
